...
class _HabitantsViewState extends State<HabitantsView> {
              ...
              return SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildFilters(theme),
                    const SizedBox(height: 20),
                    _buildDataTable(theme, canEdit || canDelete, canCreate),
                  ],
                ),
              );
            },
  ...
  Widget _buildDataTable(ThemeData theme, bool hasActions, bool canCreate) {
    final list = _notifier!.filteredHabitants;

    if (list.isEmpty) {
      final isSearching = _notifier!.searchQuery.isNotEmpty;
      return Card(
        color: Colors.white,
        elevation: 2,
        ...
        child: Padding(
          ...
          child: Center(
            child: Column(
              ...
              children: [
                Icon(
                  isSearching ? Icons.person_search_rounded : Icons.person_off,
                  size: 48,
                  color: isSearching ? Colors.amber.shade800 : Colors.grey.shade400,
                ),
                const SizedBox(height: 16),
                Text(
                  isSearching ? 'Habitante no encontrado' : 'No hay habitantes registrados',
                  style: TextStyle(
                ...
                if (isSearching) ...[
                  ...
                  Text(
                    'No se encontró ningún habitante con el criterio «${_notifier!.searchQuery}».',
                  ...
                  ElevatedButton.icon(
                    ...
                    label: const Text('Crear Habitante', style: TextStyle(fontWeight: FontWeight.bold)),
  ...
  void _showHabitanteModal({Habitante? habitante}) {
    ...
    showDialog(
      ...
      builder: (dialogCtx) => BlocProvider.value(
        value: bloc,
        child: _HabitanteFormDialog(
          habitante: habitante,
          onSave: (newHabitante) {
            if (habitante != null) {
              bloc.add(UpdateHabitanteEvent(newHabitante));
            } else {
              bloc.add(CreateHabitante(newHabitante));
            }
          },
        ),
      ),
  ...
  Future<void> _exportToExcel() async {
    ...
    await DocumentExportService.exportToExcel(
      fileName: 'habitantes.xlsx',
      sheetName: 'Habitantes',
      headers: ['Nombres', 'Apellidos', 'Cédula', 'Edad', 'Sexo', 'Teléfono', 'Sector', 'Ayuda Recibida', 'Cond. Vivienda', 'Tipo Vivienda', 'Discapacidad', 'Enf. Crónica', 'Fecha Registro'],
      data: list.map((h) {
        final birthDate = h.fechaNacimiento;
        String ageText = '-';
        if (birthDate != null) {
        ...
        final sexoText = h.sexo.isEmpty ? '-' : h.sexo[0].toUpperCase() + h.sexo.substring(1);
        return [
          h.nombres,
          h.apellidos,
          h.cedula,
